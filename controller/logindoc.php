...
class logindoc
{
    public function run($request){
        ...
        $successReturn=array("status"=>"success","username"=>"");
        ...
        $formiddoc=$db->select("docTable",["formid"],["username"=>$username]);
        $formid = $formiddoc[0]["formid"].$formid;
        $res=$db->select("docTable",[],["username"=>$username,"password"=>$password]);
        if(count($res)!=0){

            $res=$db->update("docTable",["openid"=>$openid,"formid"=>$formid],["username"=>$username]);
