...
define("APPID","wx9377d109e217cf91");
define("SECRET","a6b05ffd1eff596109e9ecda91e65876");
...
class mysql{
    ...
    //为了查询记录而特例的函数
    public function selectNote($username){
        $sql="SELECT area as position,drug as drug FROM drugTable WHERE username = {$username}";
        return $this->_execute($sql);
    }
    public function selectComments($username){
        $sql="SELECT realname as realname,edittime as edittime ,comment as comment ,cid as cid ,reply as reply FROM commentTable WHERE username = {$username}";
        return $this->_execute($sql);
    }
    public function selectCom_doc(){
        $sql="SELECT realname as realname,edittime as edittime ,comment as comment ,cid as cid ,reply as reply FROM commentTable  ";
        return $this->_execute($sql);
    }
...
function http_request($url, $data = null)
{
  ...
  //  curl_setopt($curl,CURLOPT_HTTPHEADER,array(
   ...
   // ./configure --prefix=/usr/local/php --with-config-file-path=/etc/php --enable-zip --enable-soap --with-openssl --with-curl --with-imap-ssl
   // ));
...
//二维码获取函数
function getQRcode($parm,$openid,$timesw){
    var_dump($parm);
    var_dump($openid);
    var_dump($timesw);
    echo($timesw);
   ...
   // var_dump($res);
    ...
    //$parm = urlencode($parm);
    //$timesw = urlencode($timesw);
    $url="https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=".$access_token;
    $postData=json_encode([
        "touser"=>$openid,
        "template_id"=>"adH8sPMOqMXptpAFY2K3NV-IOX34LFE9XVbSvuSFH74",
        "page"=>"pages/user_login/user_login",
        "data"=>[
            "thing1"=>[
                "value"=>"该上眼药啦"
            ],
